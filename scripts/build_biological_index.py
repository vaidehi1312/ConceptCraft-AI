...
BIOLOGICAL_FILES = [
    "metadata/animals_metadata.json",
    "metadata/medshapenet_metadata.json",
    "metadata/nih_metadata.json",
    "metadata/rcsb_metadata.json",
    "metadata/rcsb_pdb_metadata.json",
]

all_models = []

for filepath in BIOLOGICAL_FILES:
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        try:
            ...
            for i, m in enumerate(models):
                ...
                if not embed_text:
                    embed_text = "unknown biological model"
...
# Objaverse biological
with open("metadata/objaverse_bio.jsonl", "r", encoding="utf-8") as f:
    count = 0
    for i, line in enumerate(f):
        try:
            ...
            if not embed_text:
                embed_text = "unknown biological model"
            m["embed_text"]     = embed_text
            m["source_file"]    = "objaverse_bio"
            m["original_index"] = i
            all_models.append(m)
            count += 1
        except: continue
print(f"✅ Loaded {count} entries from objaverse_bio.jsonl")

print(f"\n📦 Total biological models: {len(all_models)}")
...
faiss.write_index(index, "indexes/biological.index")
with open("indexes/biological.json", "w", encoding="utf-8") as f:
    json.dump(all_models, f, indent=2, ensure_ascii=False)

print(f"\n✅ Saved:")
print(f"   indexes/biological.index  ({index.ntotal} vectors, dim={dimension})")
print(f"   indexes/biological.json   ({len(all_models)} models)")